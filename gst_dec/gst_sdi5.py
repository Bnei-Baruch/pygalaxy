...
udpsrc timeout=1000000000 port=8044 caps="application/x-rtp, media=video, payload=100, clock-rate=90000, encoding-name=VP8-DRAFT-IETF-01" ! 
...
decklinkvideosink device-number=1 mode=3''')
pipeline.set_state(Gst.State.PLAYING)
sleep(3)
bus = pipeline.get_bus()
#bus.add_signal_watch()
overlay_dict = {}
overlay_dict['8044'] = pipeline.get_by_name('overlay')
global q
q = Queue()
...
while 1:
    ...
    try:
        message=q.get(False)
        print tt_log()+" [main loop] recieved new title: "+message
    except:
        pass
    if message!='':
